...
export const BriefLitigationIndex = (props: { showHeader: boolean }) => {
  ...
  const actionBodyTemplate = (rowData: any) => {
    const goToEditPage = () => {
      navigate(`/briefs/litigations/${selectedBrief.id}/edit`);
    };
    const goToViewPage = () => {
      navigate(`/briefs/litigations/${selectedBrief.id}`);
    };
    ...
    <div>
      {props.showHeader ? (
        <div className="flex justify-between p-8">
          ...
          <Link to="/briefs/litigations/add">
            <Button label="Add Litigation" icon="pi pi-plus" outlined />
          </Link>
        </div>
      ) : (
