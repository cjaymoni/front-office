...
import {
  IVisitEntry,
  addNewVisitEntryRequest,
  getVisitEntryByIdRequest,
  updateVisitEntryRequest,
...
export const VisitEntryForm = () => {
  const navigate = useNavigate();
  const targetRef = useRef(null);
  const [isSubmitted, setIsSubmitted] = useState(false);
  const [visitEntryDetails, setVisitEntryDetails] = useState<IVisitEntry>();
  ...
  useEffect(() => {
    if (urlParams.visitEntryId) {
      const visitEntryId = urlParams.visitEntryId;
      getVisitEntryByIdRequest(visitEntryId).then((response) => {
        setVisitEntryDetails(response);
      });

      //   setVisitEntryDetails(props.visitEntryDetails);
    }
  }, [urlParams.visitEntryId]);
  ...
  const formikForm = useFormik({
    ...
    onSubmit: (values: any) => {
      ...
      if (urlParams.visitEntryId) {
        updateVisitEntryRequest(filteredObject, urlParams.visitEntryId);
      } else {
        addNewVisitEntryRequest(filteredObject).then((res) => {
